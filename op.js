...
const server = http.createServer(function(req, res) {
    if (req.url == '/') {
        ...
        fs.createReadStream('main.html').pipe(res); // Updated to use 'index.html'
    } 
    else if(req.url == '/styles.css') {
        res.writeHead('200', { 'Content-Type': 'text/css' });
        fs.createReadStream('styles.css').pipe(res); // Updated to use 'index.html'
    }
...
server.listen('10000', function() {
    console.log("Server started at 10000 on localhost");
});
